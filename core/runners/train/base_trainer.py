...
# @Function      : Base training runner with checkpointing
...
from core.runners.abstract import Runner
from core.utils.compile import configure_dynamo
from core.utils.log_utils import configure_logger
...
class Trainer(Runner):
    ...
    def __init__(self):
        ...
        self.has_disc = (
            self.cfg.model.has_disc if hasattr(self.cfg.model, "has_disc") else False
        )
        ...
        def decorator(func):
            def wrapper(self, *args, **kwargs):
                if option is None or hasattr(self.accelerator, option):
                    accelerated_func = (
                        getattr(self.accelerator, option)(func)
    ...
    def prepare_everything(self, is_dist_validation: bool = True):
        ...
        N_total_batch_size = (
            ...
            * self.cfg.train.accum_steps
        )
        self.N_global_steps_per_epoch = math.ceil(
            len(self.train_loader) / self.cfg.train.accum_steps
        )
        ...
        print(
            f"** Total: {sum(p.numel() for p in self.model.parameters() if p.requires_grad) / 1e6}M"
        )
        ...
        for sub_name, sub_module in self.accelerator.unwrap_model(
            self.model
        ).named_children():
            logger.info(
                f"** {sub_name}: {sum(p.numel() for p in sub_module.parameters() if p.requires_grad)}"
            )
        logger.info(f"=====================================")
        self.accelerator.wait_for_everyone()
    ...
    def load_model_(self, cfg):
        ...
        try:
            safetensors.torch.load_model(
                ...
                strict=(
                    cfg.saver.load_model_strict
                    if hasattr(cfg.saver, "load_model_strict")
                    else True
                ),
            )
        except:
            ...
            for key in list(state_dict):
                if "renderer.smplx_model" in key:
    ...
    def _save_checkpoint(self):
        ...
        logger.info(
            f"======== Saved checkpoint at global step {self.global_step} ========"
        )
        ...
        cur_order = ckpt_base ** math.floor(
            math.log(max_ckpt // ckpt_period, ckpt_base)
        )
        cur_idx = 0
        while cur_order > 0:
            cur_digit = max_ckpt // ckpt_period // cur_order % ckpt_base
            while (
                cur_idx < len(ckpt_dirs)
                and int(ckpt_dirs[cur_idx]) // ckpt_period // cur_order % ckpt_base
                < cur_digit
            ):
                if int(ckpt_dirs[cur_idx]) // ckpt_period % cur_order != 0:
                    shutil.rmtree(
                        os.path.join(os.path.dirname(ckpt_dir), ckpt_dirs[cur_idx])
                    )
    ...
    @control("on_main_process")
    def log_scalar_kwargs(
        self, epoch: int = None, step: int = None, split: str = None, **scalar_kwargs
    ):
    ...
    def log_images_each_process(
        self, values: dict, step: int | None = None, log_kwargs: dict | None = {}
    ):
        for tracker in self.accelerator.trackers:
            if hasattr(tracker, "log_images"):
                tracker.log_images(
                    values, step=step, **log_kwargs.get(tracker.name, {})
                )
    ...
    def log_optimizer(
        ...
        if not attrs:
            logger.warning(
                "No optimizer attributes are provided, nothing will be logged"
            )
        if not group_ids:
            logger.warning(
                "No optimizer group ids are provided, nothing will be logged"
            )
        for attr in attrs:
            assert attr in [
                "lr",
                "momentum",
                "weight_decay",
            ], f"Invalid optimizer attribute {attr}"
            for group_id in group_ids:
                self.accelerator.log(
                    {
                        f"opt/{attr}/{group_id}": self.optimizer.param_groups[group_id][
                            attr
                        ]
                    },
                    log_progress,
