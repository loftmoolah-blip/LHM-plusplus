...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            x = (
                self.relu(self.bn(self.linear(x))).transpose(1, 2).contiguous()
            )  # (m, c, nsample)
...
class PointTransformerBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, share_planes=8, nsample=16):
        ...
        self.bn3 = nn.LayerNorm(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, c=51, k=13):
        super().__init__()
        self.c = c
        # self.in_planes, planes = c, [32, 64, 128, 256, 512]
        self.in_planes, planes = c, [128, 256, 512]
        fpn_planes, fpnhead_planes, share_planes = 128, 64, 8
        stride, nsample = [1, 4, 4], [8, 16, 16]
        ...
        self.dec3 = self._make_dec(
            block, planes[2], 2, share_planes, nsample=nsample[2], is_head=True
        )  # transform p3
...
if __name__ == "__main__":
    ...
    x = torch.randn(20000, 3).cuda()
    c = torch.randn(20000, 48).cuda()
    N = x.shape[0]
    o = torch.IntTensor([N]).cuda()
