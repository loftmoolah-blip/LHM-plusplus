...
class XUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        enc_output = self.encoder.forward_intermediates(
            x, stop_early=True, intermediates_only=True
        )
    ...
    def __init__(
        ...
        if freeze:
            if modulation_dim is not None:
                raise ValueError(
                    "Modulated SwinUnetWrapper requires training, freezing is not allowed."
                )
            self._freeze()
