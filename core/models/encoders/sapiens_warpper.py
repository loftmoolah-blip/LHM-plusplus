...
class SapiensWrapper(nn.Module):
    def __init__(
        ...
        self.patch_size = (
            resolution // self.downsample_ratio,
            resolution // self.downsample_ratio,
        )  # h, w
    ...
    def _preprocess_image(
        self, image: torch.tensor, resolution: int = 1024, required_norm=True
    ) -> torch.Tensor:
...
class SapiensClassWrapper(SapiensWrapper):
    def __init__(
        self,
        model_name: str,
        freeze: bool = True,
        ...
        **kwargs,
        ...
        super(SapiensClassWrapper, self).__init__(
            model_name,
            freeze=freeze,
        ...
        self.class_model = Dinov2Wrapper(**kwargs["encoder_params"])

        self.warp = nn.Sequential(
            linear(1536, 768),
            nn.SiLU(),
            linear(768, encoder_feat_dim),
        )
        self.outnorm = nn.LayerNorm(encoder_feat_dim)
    ...
    def forward(self, image: torch.Tensor, mod: torch.Tensor = None):
        ...
        # mod: [N, D] or None
        # RGB image with [0,1] scale and properly sized
        ori_image = self._preprocess_image(image, self.resolution, required_norm=False)
        image = kornia.enhance.normalize(ori_image, self.mean, self.std)
        # NOTE that, only supports
        patch_h, patch_w = (
            image.shape[-2] // 16,
            image.shape[-1] // 16,
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            (out_local,) = self.model(image)
            cls_tokens = self.class_model._forward_class_tokens(ori_image)

        out_global = None
        if out_global is not None:
            raise NotImplementedError("Global feature is not supported yet.")
        else:
            features = out_local.permute(0, 2, 3, 1).flatten(1, 2)

        # cls_tokens = F.pad(
        #     cls_tokens, (0, features.shape[-1] - cls_tokens.shape[-1], 0, 0, 0, 0)
        # )  # the same as sd3, learnable

        features = self.outnorm(self.warp(features))
        ret = torch.cat([cls_tokens, features], dim=1)
