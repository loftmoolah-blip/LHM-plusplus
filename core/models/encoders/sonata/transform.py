...
@TRANSFORMS.register_module()
class ToPvPoints(object):
    def __init__(self):
        self.mapping = dict(
            neutral_coords="coord",
            neutral_normals="normal",
        )
    ...
    def __call__(self, data_dict):
        ...
        for key in self.mapping.keys():
            ...
            query_feats = data_dict[key]
            if 3 == query_feats.dim():
                query_feats = query_feats.squeeze(0)
            data_ret[new_key] = query_feats.detach().cpu().numpy()
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        """coord, color, normal, instance, segment."""
        ...
        if self.mode == "train":  # train mode
            ...
            # return keys: ['coord', 'color', 'normal', 'instance', 'segment', 'index_valid_keys', 'inverse', 'grid_coord']
...
def default():
    config = [
        dict(type="ToPvPoints"),
        dict(type="CenterShift", apply_z=True),
        dict(
            type="GridSample",
            # grid_size=0.005,  # default
            grid_size=0.01,  # default
            hash_type="fnv",
            mode="train",
