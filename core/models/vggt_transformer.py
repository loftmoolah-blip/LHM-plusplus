...
from core.models.vggt.layers.vision_transformer import (
    vit_base,
    vit_giant2,
    vit_large,
    vit_small,
...
class VGGTAggregator(nn.Module):
    ...
    def __init__(
        ...
        aa_block_size=1,
        qk_norm=True,
        rope_freq=100,  # base frequency for 2D rotary embedding
        init_values=0.01,
    ):
        super().__init__()

        # encoder is defined in outside
        ...
        self.frame_blocks = nn.ModuleList(
            [
                block_fn(
                    ...
                    proj_bias=proj_bias,
                    ffn_bias=ffn_bias,
                    init_values=init_values,  # scale ratio
                    qk_norm=qk_norm,  # qk normalization
                    rope=self.rope,  # rotary position embedding
                )
        ...
        # The same applies for register tokens, attention requires the register

        self.pose_token = nn.Parameter(torch.randn(1, 2, 1, embed_dim))
    ...
    def prepare_image_tokens(
        self, patch_tokens: torch.Tensor, latent_size: Tuple[int, int]
    ):
        ...
        if self.rope is not None:
            pos = self.position_getter(B * S, H, W, device=pose_token.device)
    ...
    def _process_frame_attention(self, tokens, B, S, P, C, frame_idx, pos=None):
        ...
        return:
            ...
            intermediate_metrics (using key)
        """
        ...
        intermediates = []
        intermediates_metric = []
        # by default, self.aa_block_size=1, which processes one block at a time

        for _ in range(self.aa_block_size):
            tokens, metric = self.frame_blocks[frame_idx](tokens, pos=pos)
            frame_idx += 1

            intermediates.append(tokens.view(B, S, P, C))
            intermediates_metric.append(metric.view(B, S, P, -1))

        return tokens, frame_idx, intermediates, intermediates_metric
...
if __name__ == "__main__":
    vggt_model = VGGTAggregator()

    x = torch.randn(1, 25, 3, 350, 518).float()
    vggt_model.cuda()

    with torch.no_grad():
        x = x.cuda()
        out, patch_idx = vggt_model(x)
