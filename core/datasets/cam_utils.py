...
# @Function      : Camera extrinsic/intrinsic utilities
...
def camera_normalization_objaverse(
    normed_dist_to_center, poses: torch.Tensor, ret_transform: bool = False
):
...
def build_camera_standard(RT: torch.Tensor, intrinsics: torch.Tensor):
    ...
    I = torch.stack(
        [
            ...
            torch.tensor([[0, 0, 1]], dtype=torch.float32, device=RT.device).repeat(
                RT.shape[0], 1
            ),
        ],
        dim=1,
...
def create_intrinsics(
    ...
    if c is not None:
        ...
        ), "cx/cy must be provided when c is not provided"
