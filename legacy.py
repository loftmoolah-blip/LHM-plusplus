...
# @Function      : Legacy StyleGAN/EG3D model loading utilities
...
def convert_to_rgb(state_ros, state_nv, ros_name, nv_name):
    state_ros[f"{ros_name}.conv.weight"] = state_nv[
        f"{nv_name}.torgb.weight"
    ].unsqueeze(0)
    state_ros[f"{ros_name}.bias"] = (
        state_nv[f"{nv_name}.torgb.bias"].unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
    )
    state_ros[f"{ros_name}.conv.modulation.weight"] = state_nv[
        f"{nv_name}.torgb.affine.weight"
    ]
    state_ros[f"{ros_name}.conv.modulation.bias"] = state_nv[
        f"{nv_name}.torgb.affine.bias"
    ]
...
def convert_conv(state_ros, state_nv, ros_name, nv_name):
    ...
    state_ros[f"{ros_name}.conv.modulation.weight"] = state_nv[
        f"{nv_name}.affine.weight"
    ]
    state_ros[f"{ros_name}.conv.modulation.bias"] = state_nv[f"{nv_name}.affine.bias"]
    state_ros[f"{ros_name}.noise.weight"] = state_nv[
        f"{nv_name}.noise_strength"
    ].unsqueeze(0)
...
def convert_blur_kernel(state_ros, state_nv, level):
    ...
    state_ros[f"convs.{2*level}.conv.blur.kernel"] = (
        4 * state_nv["synthesis.b4.resample_filter"]
    )
    state_ros[f"to_rgbs.{level}.upsample.kernel"] = (
        4 * state_nv["synthesis.b4.resample_filter"]
    )
