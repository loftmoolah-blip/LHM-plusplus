...
def write_json(path, x):
    """Persist a Python mapping to a JSON file on disk.

    This utility serializes the input mapping to a human-readable JSON
    representation (with indentation) and writes it to the specified path.

    Parameters
    ----------
    path : str
        Destination file path where the JSON content will be written.
    x : dict
        The dictionary-like object to serialize.

    Returns
    -------
    None
        This function performs I/O side effects and returns ``None``.

    Notes
    -----
    The function overwrites the destination file if it already exists.
    """
    import json
...
def psnr_compute(
    ...
    results_data : str
        Directory containing result/prediction images.
...
if __name__ == "__main__":
    ...
    save_folder = os.path.join(f"./exps/metrics_{opt.pre}", "psnr_results", *target_key)
    ...
    for item in tqdm(items):
        ...
        target_item_folder = os.path.join(target_folder, item)
